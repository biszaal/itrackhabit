...
export interface SupabaseHabit {
  ...
  stacked_habit_id?: string;
  stack_order: number;
  mini_version?: string;
  reward_activity?: string;
  is_active: boolean;
  archived_at?: string;
  created_at: string;
  updated_at: string;
...
class SupabaseService {
  ...
  constructor() {
    ...
    const isValidKey = supabaseAnonKey && 
                      ...
                      supabaseAnonKey.length > 50; // Supabase keys are quite long

    if (!isValidUrl || !isValidKey) {
      console.log('📱 Running in offline-only mode');
      console.log('💡 To enable cloud sync, configure valid Supabase credentials in .env.local');
    ...
    try {
      this.supabase = createClient(supabaseUrl, supabaseAnonKey, {
        ...
        realtime: {
          params: {
            eventsPerSecond: 5, // Limit real-time events for better performance
          },
        },
      });

      console.log('🔗 SupabaseService initialized with cloud sync enabled');
    } catch (error) {
  ...
  private convertHabitToSupabase(habit: Habit): Partial<SupabaseHabit> {
    return {
      ...
      category: 'General', // Map from habit properties as needed
      frequency: habit.frequency,
      icon_emoji: '⭐', // Default or map from habit.iconEmoji
      color: habit.color || '#6B9EFF',
      target_value: habit.targetConfig?.targetValue || 1,
      target_unit: habit.targetConfig?.unit || 'times',
      is_time_based: habit.targetConfig?.isTimeBased || false,
      stack_order: 0,
      is_active: !habit.deletedAt,
      archived_at: habit.deletedAt,
  ...
  private convertSupabaseToHabit(supabaseHabit: SupabaseHabit): Habit {
    return {
      ...
      notes: supabaseHabit.description || '',
      ...
      targetConfig: {
        hasTarget: true,
        targetValue: supabaseHabit.target_value,
        unit: supabaseHabit.target_unit,
        isTimeBased: supabaseHabit.is_time_based,
      },
      createdAt: supabaseHabit.created_at,
      updatedAt: supabaseHabit.updated_at,
      deletedAt: supabaseHabit.archived_at,
    };
  }
