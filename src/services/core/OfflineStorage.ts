...
class OfflineStorageService {
  ...
  private async checkAndFixSchema(): Promise<void> {
    ...
    try {
      ...
      const habitResult = await this.db.getAllAsync('PRAGMA table_info(habits)');
      ...
      if (!habitColumns.includes('targetConfig')) {
        ...
        await this.db.execAsync('ALTER TABLE habits ADD COLUMN targetConfig TEXT');
      }
  ...
  async getHabits(userId: string): Promise<Habit[]> {
    ...
    console.log('📊 OfflineStorage: Mapped habits:', habits.map(h => ({ id: h.id, title: h.title })));
    return habits;
  }
  ...
  async getAllHabits(userId: string): Promise<Habit[]> {
    ...
    console.log('🔍 OfflineStorage: Getting ALL habits (including deleted) for userId:', userId);
    const result = await this.db.getAllAsync(
      `SELECT * FROM habits WHERE userId = ? ORDER BY createdAt DESC`,
      [userId]
    );

    console.log('📊 OfflineStorage: Found all habits in database:', result.length);
    const habits = result.map(row => this.mapRowToHabit(row as any));
    return habits;
  }
  ...
  async getHabitById(habitId: string, includeDeleted: boolean = false): Promise<Habit | null> {
    ...
    const whereClause = includeDeleted ? `id = ?` : `id = ? AND deletedAt IS NULL`;
  ...
  async deleteHabit(habitId: string): Promise<void> {
    ...
    console.log('📊 Habit before deletion:', habitBefore ? habitBefore.title : 'NOT FOUND');
    ...
    console.log('📊 Total habits after soft delete (including deleted):', habitAfter.length);
  ...
  async getHabitProgress(habitId: string, startDate?: string, endDate?: string): Promise<HabitProgress[]> {
    ...
    console.log('📊 getHabitProgress found', result.length, 'entries for habit', habitId);
  ...
  async clearUserData(userId: string): Promise<void> {
    ...
    try {
      ...
      await this.db.runAsync(
        `DELETE FROM habits WHERE userId = ?`,
        [userId]
      );
      
      // Clear sync queue for this user
      await this.db.runAsync(
        `DELETE FROM sync_queue WHERE data LIKE ?`,
        [`%"userId":"${userId}"%`]
      );
