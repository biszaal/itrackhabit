...
export interface HabitRecommendation {
  id: string;
  type: 'new_habit' | 'habit_improvement' | 'time_optimization' | 'streak_recovery';
  title: string;
  description: string;
...
class AIInsightsService {
  ...
  async generateHabitRecommendations(): Promise<HabitRecommendation[]> {
    ...
    const averageCompletionRate = completionRates.reduce((sum, rate) => sum + rate, 0) / completionRates.length || 0;
    ...
    newRecommendations.push(...this.generateImprovementSuggestions(habits, averageCompletionRate));
  ...
  private analyzeHabitCategories(habits: HabitWithStats[]): Record<string, number> {
  ...
  private generateComplementaryHabits(categories: Record<string, number>): HabitRecommendation[] {
    const recommendations: HabitRecommendation[] = [];
    const totalHabits = Object.values(categories).reduce((sum, count) => sum + count, 0);

    // Suggest missing essential categories
    const essentialCategories = [
      { type: 'health', title: 'Morning Exercise', description: 'Start your day with 20 minutes of physical activity', emoji: '💪' },
      { type: 'mindfulness', title: 'Daily Meditation', description: 'Practice 10 minutes of mindfulness meditation', emoji: '🧘' },
      { type: 'learning', title: 'Read for Growth', description: 'Read 15 pages of a personal development book daily', emoji: '📚' },
      { type: 'productivity', title: 'Plan Tomorrow', description: 'Spend 5 minutes planning the next day before bed', emoji: '📝' },
    ];
    ...
    essentialCategories.forEach(category => {
      if (!categories[category.type] || categories[category.type] === 0) {
        recommendations.push({
          ...
          estimatedImpact: 'High - This habit complements your existing routine perfectly',
  ...
  private generateImprovementSuggestions(habits: HabitWithStats[], averageRate: number): HabitRecommendation[] {
    const recommendations: HabitRecommendation[] = [];

    // Find habits with low completion rates
    const strugglingHabits = habits.filter(h => h.completionRate < averageRate - 20 && h.completionRate < 70);
    ...
    strugglingHabits.forEach(habit => {
      recommendations.push({
        ...
        description: `Your completion rate for this habit is ${Math.round(habit.completionRate)}%. Let's optimize it!`,
  ...
  private generateTimeOptimizations(habits: HabitWithStats[]): HabitRecommendation[] {
    const recommendations: HabitRecommendation[] = [];
    ...
    if (similarHabits.length > 1) {
      recommendations.push({
        ...
        priority: 'medium',
        confidence: 72,
        suggestedAction: `Try doing ${similarHabits.map(h => h.title).join(', ')} in sequence`,
        relatedHabits: similarHabits.map(h => h.id),
        estimatedImpact: 'Medium - Habit stacking can improve consistency by 30%',
  ...
  private findSimilarHabits(habits: HabitWithStats[]): HabitWithStats[] {
    ...
    const categories = this.analyzeHabitCategories(habits);
    const largestCategory = Object.keys(categories).reduce((a, b) => categories[a] > categories[b] ? a : b);
    
    return habits.filter(h => (h.type || 'general') === largestCategory).slice(0, 3);
  }
  ...
  private async analyzeCompletionPatterns(habits: HabitWithStats[]): Promise<HabitInsight[]> {
    ...
    for (const habit of habits) {
      ...
      if (weeklyCompletion.weekends < weeklyCompletion.weekdays - 30) {
        insights.push({
          ...
          description: `You're ${Math.round(weeklyCompletion.weekdays - weeklyCompletion.weekends)}% less likely to complete "${habit.title}" on weekends`,
          data: {
            metric: 'Weekend vs Weekday completion',
            value: `${Math.round(weeklyCompletion.weekends)}% vs ${Math.round(weeklyCompletion.weekdays)}%`,
            trend: 'declining',
            comparison: 'weekdays',
          },
          actionable: true,
      ...
      if (habit.longestStreak > habit.currentStreak * 2 && habit.currentStreak < 7) {
        insights.push({
  ...
  private getWeeklyCompletionRate(progress: HabitProgress[]): { weekdays: number; weekends: number } {
    ...
    return {
      weekdays: totalWeekdays > 0 ? (weekdayCompletions / totalWeekdays) * 100 : 0,
      weekends: totalWeekends > 0 ? (weekendCompletions / totalWeekends) * 100 : 0,
    };
  }

  private async analyzeTimingPatterns(habits: HabitWithStats[]): Promise<HabitInsight[]> {
    const insights: HabitInsight[] = [];
    ...
    const morningHabits = highPerformingHabits.filter(h => h.title.toLowerCase().includes('morning') || h.title.toLowerCase().includes('exercise'));
    ...
    if (morningHabits.length > 0) {
      insights.push({
        ...
        description: `You're ${Math.round((morningHabits.length / highPerformingHabits.length) * 100)}% more successful with morning habits`,
        data: {
          metric: 'Morning habit success rate',
          value: `${Math.round(morningHabits.reduce((sum, h) => sum + h.completionRate, 0) / morningHabits.length)}%`,
          trend: 'stable',
        },
        actionable: true,
  ...
  private async analyzePerformanceTrends(habits: HabitWithStats[]): Promise<HabitInsight[]> {
    const insights: HabitInsight[] = [];

    // Overall performance insight
    const averageCompletion = habits.reduce((sum, h) => sum + h.completionRate, 0) / habits.length || 0;
    ...
    if (averageCompletion > 70) {
      insights.push({
        ...
        description: `You're maintaining ${Math.round(averageCompletion)}% completion rate across ${habits.length} habits - excellent consistency!`,
        data: {
  ...
  async getOptimalTiming(habitId: string): Promise<OptimalTiming> {
    ...
    const recommendations = this.getTimeRecommendationsByType(habit.type || 'general');
  ...
  private getTimeRecommendationsByType(type: string): {
    optimal: { hour: number; minute: number; confidence: number; reason: string }[];
    avoid: { hour: number; minute: number; reason: string }[];
  } {
    const recommendations: Record<string, any> = {
      health: {
        optimal: [
          { hour: 7, minute: 0, confidence: 90, reason: 'Morning energy levels are highest' },
          { hour: 18, minute: 0, confidence: 75, reason: 'Post-work stress relief' },
        ],
        avoid: [
      ...
      productivity: {
        optimal: [
          { hour: 9, minute: 0, confidence: 85, reason: 'Peak cognitive performance' },
          { hour: 14, minute: 30, confidence: 70, reason: 'Afternoon focus window' },
        ],
        avoid: [
    ...
    return recommendations[type] || {
      optimal: [
        { hour: 8, minute: 0, confidence: 70, reason: 'Morning routine integration' },
        { hour: 19, minute: 0, confidence: 65, reason: 'Evening routine integration' },
      ],
      avoid: [
        { hour: 23, minute: 0, reason: 'Too late in the day' },
      ],
    };
  }
  ...
  async generatePersonalizedCoaching(): Promise<PersonalizedCoaching[]> {
    ...
    if (habits.length >= 3) {
      newCoaching.push({
        ...
        message: 'Try linking your habits together! After you finish one habit, immediately start the next. This creates powerful behavioral chains.',
      ...
      for (let j = i + 1; j < habits.length; j++) {
        ...
        if (Math.abs(correlation) > 0.3) {
          correlations.push({
            ...
            insight: this.generateCorrelationInsight(habitA, habitB, correlation),
          });
        }
      }
    }
  ...
  private calculateMockCorrelation(habitA: HabitWithStats, habitB: HabitWithStats): number {
    ...
    const completionSimilarity = 1 - Math.abs(habitA.completionRate - habitB.completionRate) / 100;
    return Math.min(0.9, categoryMatch + completionSimilarity * 0.6) * (Math.random() > 0.5 ? 1 : -1);
  }

  private generateCorrelationInsight(habitA: HabitWithStats, habitB: HabitWithStats, correlation: number): string {
    if (correlation > 0) {
